...
export default defineConfig({
  entry: {
    ...
    // Its own entry so redux-persist/AsyncStorage are only reachable — and so
    // only need installing — when the app actually enabled persistence.
    'runtime/persistAdapter': 'src/runtime/persistAdapter.ts',
  },
  format: ['cjs', 'esm'],
  dts: true,
